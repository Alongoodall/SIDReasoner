...
class MultiEvalTrainer(transformers.Trainer):
    ...
    def evaluate(
        ...
        for name, dataset in self.extra_eval_sets.items():
            ...
            # Note: metrics.update() is sufficient; self.log() here would cause double-logging
...
def train(
    ...
    # Determine main rank early — used throughout for single-process-only operations
    main_rank = (
    ...
    category = category_dict.get(category, "items")
    ...
    # wandb.init() is here so num_new_tokens is defined and included in the config
    if main_rank:
        wandb.init(
            project=wandb_project,
            name=wandb_run_name,
            tags=[category, "stage2", "reasoning-activation"],
            config={
                ...
                "train_new_token_embeddings_only": train_new_token_embeddings_only,
                "train_from_scratch": train_from_scratch,
            },
        )

    if train_new_token_embeddings_only and num_new_tokens > 0:
        vocab_size = len(tokenizer)
        new_token_indices = list(range(vocab_size - num_new_tokens, vocab_size))
        print("Restricting training to new token ids.")
    ...
    # Single-pass dataset construction (faster than from_dict) with one shuffle each
    hf_train_dataset = HFDataset.from_list([dict(item) for item in train_data]).shuffle(seed=seed)
    hf_val_dataset = HFDataset.from_list([dict(item) for item in val_data_sid_prediction]).shuffle(seed=seed)
    hf_eval_dataset_title2sid = HFDataset.from_list([dict(item) for item in val_data_title2sid_translation]).shuffle(seed=seed)
    hf_eval_dataset_sid2title = HFDataset.from_list([dict(item) for item in val_data_sid2title_translation]).shuffle(seed=seed)

    extra_eval_sets = {
        "title2sid": hf_eval_dataset_title2sid,
        "sid2title": hf_eval_dataset_sid2title,
    }

    print(hf_train_dataset)
    print(hf_val_dataset)
    print(hf_eval_dataset_title2sid)
    print(hf_eval_dataset_sid2title)
